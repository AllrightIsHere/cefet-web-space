...
const btnPreviousEl = document.querySelector('#anterior');
const btnNextEl = document.querySelector('#proximo');

let currentIndex = 0;

function changeImg() {
  const imgEl = document.querySelector('#slide');
  imgEl.src = servidorDasImagens + '/' + imagens[currentIndex].arquivo;
  imgEl.alt = imagens[currentIndex].descricao;
}

btnPreviousEl.addEventListener('click', () => {
  currentIndex = currentIndex - 1 >= 0 ? currentIndex - 1 : imagens.length - 1;
  changeImg();
});
